 ...
 * @Description:
 * synchronized 关键字基本应用 验证sync的加锁位置
 * 1，静态方法枷锁
 * 2.方法区枷锁
...
public class SyncDemo {
    ...
    public static synchronized void t5(){ //相当于锁定 T。class
        count2--;
        try {
            Thread.sleep(3000);
        } catch (InterruptedException e) {
    ...
    public static void main(String[] args){
         ...
         * 线程t5，对SyncDemo.Class对象加锁
         */
        new Thread(t::t1,"t1").start();
        new Thread(t::t2,"t2").start();
        new Thread(t::t3,"t3").start();
        new Thread(SyncDemo::t4,"t4").start();
        new Thread(SyncDemo::t5,"t5").start();
    }
